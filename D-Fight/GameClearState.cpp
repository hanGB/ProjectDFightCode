...
CGameClearState::CGameClearState()
{
...
void CGameClearState::NormalRender()
{
	// 스카이 박스 렌더링
	m_pTimeOfDaySkybox->RenderSkybox(DirectionalLight());
}

void CGameClearState::UIRender()
{
...
void CGameClearState::LoadData()
{
	m_WhiteRectTextureID = (int)Renderer()->ReadPNGTextureFile("./resource/ui/whiteRect.png");

	m_gameclearBGMID = SoundPlayer()->LoadSound("./resource/sound/BGM/clearBGM.mp3");

	// bgm 설정
	m_pBGM = SoundPlayer()->SetBackGroundMusic(m_gameclearBGMID, 0.5f);

	// 타이틀 로고
	m_LogoTextureID = (int)Renderer()->ReadPNGTextureFile("./resource/ui/Gameclear.png");

}

void CGameClearState::AddObjectToManager()
{
	m_ButtonVector.push_back(new CButton(L"Back to Field"));
	m_ButtonVector.push_back(new CButton(L"Exit"));
}

void CGameClearState::ProcessInputs(float elapsedTimeInSec, GameInputs* inputs)
{
...
void CGameClearState::Update(float elapsedTimeInSec)
{
	m_pTimeOfDaySkybox->Update(elapsedTimeInSec * 2, DirectionalLight());
}

void CGameClearState::RenderScene()
{
	Camera()->UseCamera();
...
void CGameClearState::ButtonSelectInputs(float elapsedTimeInSec, GameInputs* inputs)
{
...
void CGameClearState::UseButton(int menu)
{
	switch (menu) {
	case 0:
